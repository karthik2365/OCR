...
// FIX: Define the OverpassElement type here to replace 'any' in the fetching logic
...
      // FIX: Use specific type assertion to avoid 'any' error
...
            .map((element: OverpassElement, index: number) => { // FIX: Use OverpassElement type
...
              } as NaturalFeature
            })
            .filter((feature: NaturalFeature) => feature.lat && feature.lon)
        }

        setFeatures(featureData)
        onFeaturesFetched?.(featureData)
      } catch (err) { // FIX: Removed 'err' from catch parameter
...
    fetchFeatures()
  }, [destinationCoordinates, onFeaturesFetched]) // FIX: Added 'onFeaturesFetched' to deps
...
    fetchRoute()
  }, [originCoordinates, destinationCoordinates, loading]) // FIX: Added 'loading' to deps
...
  const decodePolyline = (str: string, precision: number): [number, number][] => {
    ...
    const coordinates: [number, number][] = [] // FIX: Changed 'let' to 'const'
    const factor = Math.pow(10, precision || 5) // FIX: Changed 'let' to 'const'
